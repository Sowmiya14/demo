@extends('main.layout.master')

@section('title')Sri Shanmugha College of Pharmacy - Vision & Mission @endsection


@section('Description') Sri Shanmugha College of Pharmacy was established in the year 2017 by the industrialist and philanthropist Thiru.K.Shanmugham. The college is recognized by the Government of Tamil Nadu and affiliated to The Tamil Nadu Dr.MGR Medical University. @endsection

@section('MobileTitle') Vision & Mission @endsection


@section('About')active @endsection


@section('BreadCrumb')
    <section class="breadcrumb-classic context-dark">
        <div class="shell section-30 section-sm-top-70 section-sm-bottom-60">
            <h1 class="veil reveal-sm-block">Vision & Mission</h1>
            <div class="offset-sm-top-35">
                <ul class="list-inline list-inline-lg list-inline-dashed p">
                    <li><a href="{{ url('/') }}">Home</a></li>
                    <li>Vision & Mission</li>
                </ul>
            </div>
        </div>
    </section>
@endsection


@section('content')
    <section class="section-grid-demonstration">
        <div class="shell">
            <h2 class="text-bold">Vision & Mission</h2>
            <hr class="divider bg-madison">
            <div class="row offset-top-60 text-sm-left">
                <div class="col-xs-12">
                    <div class="bg-lightest section-30 inset-left-30 inset-right-30">
                        <div>
                            <h6 class="text-bold">Our Vision</h6>
                            <p> We educate students to become competent Pharmacists and the graduates will be dedicated to serve the communities. To be the best among the colleges of pharmacy with attitudes, skills, knowledge and to become successful centre of creative solutions. Our institution shall become the Center of Excellence in Pharmaceutical Education and Research by developing innovative teaching practice and advancing research activities.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row offset-top-60 text-sm-left">
                <div class="col-xs-12">
                    <div class="bg-lightest section-30 inset-left-30 inset-right-30">
                        <div>
                            <h6 class="text-bold">Our Mission</h6>
                            <div class="quote-body">
                                <center><q>“WE CARE HEALTH”</q></center>
                            </div>
                            <p> Our Mission is to provide a strong foundation in the Knowledge, Integration and application of the Social welfare and Clinical Sciences to transform students into practitioners. We help in the development of students who will impact the health care needs of people in the Region, State, Nation and will enable them to shine in their life.</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row offset-top-60 text-sm-left">
                <div class="col-xs-12">
                    <div class="bg-lightest section-30 inset-left-30 inset-right-30">
                        <div>
                            <h6 class="text-bold">Objective</h6>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i>&ensp;&ensp;&ensp;Nurture of Quality in Pharmaceutical Education</p>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i>&ensp;&ensp;&ensp;Creating Innovative Pharma teaching practices</p>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i>&ensp;&ensp;&ensp;Providing Continuous Education Programme for practicing pharmacists</p>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i>&ensp;&ensp;&ensp;Promotes Research & Development</p>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i>&ensp;&ensp;&ensp;Health Care Service for the welfare of Community</p>
                            <p><i class="fa fa-hand-o-right" aria-hidden="true"></i>&ensp;&ensp;&ensp;Service to the industry by producing quality pharmacists</p>
                        </div>
                    </div>
                </div>
            </div>

        </div>
    </section>


@endsection